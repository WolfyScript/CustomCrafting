...
public class DebugSubCommand extends AbstractSubCommand {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull String var3, @NotNull String[] var4) {
        if (sender instanceof Player p && ChatUtils.checkPerm(p, "customcrafting.cmd.debug")) {
            ...
            boolean currentSetting = customCrafting.getConfigHandler().getConfig().getBoolean("debug");
            if (currentSetting) {
                customCrafting.getConfigHandler().getConfig().set("debug", false);
                api.getLogger().setLevel(Level.ALL);
            } else {
                customCrafting.getConfigHandler().getConfig().set("debug", true);
                api.getLogger().setLevel(Level.INFO);
            }
            api.getChat().sendMessage(p, "Set Debug to: " + !currentSetting);
        }
        return true;
    }
