...
public class SQLDatabaseLoader extends DatabaseLoader {
    ...
    public SQLDatabaseLoader(CustomCrafting customCrafting) {
        super(customCrafting, new BukkitNamespacedKey(customCrafting.getApi(), "database_loader"));
    ...
    public void loadRecipes() {
        ...
        try (PreparedStatement recipesQuery = dataBase.open().prepareStatement("SELECT * FROM customcrafting_recipes")) {
            ...
            while (resultSet.next()) {
                ...
                NamespacedKey namespacedKey = new BukkitNamespacedKey(customCrafting.getApi(), namespace + "/" + key);
            ...
            while (resultSet.next()) {
                ...
                if (namespace != null && key != null && data != null && !data.equals("{}")) {
                    ...
                    if (isReplaceData() || !api.getRegistries().getCustomItems().has(namespacedKey)) {
                        try {
                            api.getRegistries().getCustomItems().register(new BukkitNamespacedKey(customCrafting.getApi(), namespace + "/" + key), customCrafting.getApi().getJacksonMapperUtil().getGlobalMapper().readValue(data, CustomItem.class));
