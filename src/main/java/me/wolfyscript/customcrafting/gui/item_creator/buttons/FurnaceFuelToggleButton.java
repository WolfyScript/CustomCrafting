...
public class FurnaceFuelToggleButton extends ToggleButton<CCCache> {
    ...
    public FurnaceFuelToggleButton(String id, Material material) {
        super("fuel." + id, (cache, guiHandler, player, guiInventory, i) -> cache.getItems().getItem().getAllowedBlocks().contains(Material.FURNACE), new ButtonState<>("fuel." + id + ".enabled", material, (ItemsButtonAction) (testCache, items, guiHandler, player, inventory, i, event) -> {
