...
public class AttributeModeButton extends ActionButton {
    ...
    public AttributeModeButton(AttributeModifier.Operation operation, String headURLValue) {
        super("attribute."+operation.toString().toLowerCase(Locale.ROOT), new ButtonState("attribute."+operation.toString().toLowerCase(Locale.ROOT), PlayerHeadUtils.getViaURL(headURLValue), (guiHandler, player, inventory, i, inventoryClickEvent) -> {
