...
public class RecipeCreatorCrafting extends RecipeCreator {
    ...
    public void onInit() {
        ...
        btnB.toggle(ClusterRecipeCreator.SHAPELESS).stateFunction((holder, cache, slot) -> cache.getRecipeCreatorCache().getCraftingCache().isShapeless()).enabledState(s -> s.cluster(getCluster()).subKey("enabled").icon(PlayerHeadUtils.getViaURL("f21d93da43863cb3759afefa9f7cc5c81f34d920ca97b7283b462f8b197f813")).action((holder, cache, btn, slot, details) -> {
        ...
        btnB.toggle(ClusterRecipeCreator.MIRROR_HORIZONTAL).stateFunction((holder, cache, slot) -> cache.getRecipeCreatorCache().getCraftingCache().isMirrorHorizontal()).enabledState(s -> s.cluster(getCluster()).subKey("enabled").icon(PlayerHeadUtils.getViaURL("956a3618459e43b287b22b7e235ec699594546c6fcd6dc84bfca4cf30ab9311")).action((holder, cache, btn, slot, details) -> {
        ...
        btnB.toggle(ClusterRecipeCreator.MIRROR_VERTICAL).stateFunction((holder, cache, slot) -> cache.getRecipeCreatorCache().getCraftingCache().isMirrorVertical()).enabledState(s -> s.cluster(getCluster()).subKey("enabled").icon(PlayerHeadUtils.getViaURL("882faf9a584c4d676d730b23f8942bb997fa3dad46d4f65e288c39eb471ce7")).action((holder, cache, btn, slot, details) -> {
        ...
        btnB.toggle(ClusterRecipeCreator.MIRROR_ROTATION).stateFunction((holder, cache, slot) -> cache.getRecipeCreatorCache().getCraftingCache().isMirrorRotation()).enabledState(s -> s.cluster(getCluster()).subKey("enabled").icon(PlayerHeadUtils.getViaURL("e887cc388c8dcfcf1ba8aa5c3c102dce9cf7b1b63e786b34d4f1c3796d3e9d61")).action((holder, cache, btn, slot, details) -> {
