    ...
    public CookingContainerButton(int inputSlot, CustomCrafting customCrafting) {
        super("cooking.container_" + inputSlot, new ButtonState("", Material.AIR, new ButtonActionRender() {
            ...
            public boolean run(GuiHandler guiHandler, Player player, Inventory inventory, int slot, InventoryClickEvent event) {
                ...
                if (event.isRightClick() && event.isShiftClick()) {
                    ...
                    Bukkit.getScheduler().runTask(customCrafting, () -> {
                        CustomItem customItem = inventory.getItem(slot) != null && !inventory.getItem(slot).getType().equals(Material.AIR) ? CustomItem.getReferenceByItemStack(inventory.getItem(slot)) : new CustomItem(Material.AIR);
                        if(inputSlot == 0){
            ...
            public ItemStack render(HashMap<String, Object> hashMap, GuiHandler guiHandler, Player player, ItemStack itemStack, int i, boolean b) {
                ...
                if(inputSlot == 0 && !InventoryUtils.isCustomItemsListEmpty(cooking.getSource())){
                    return cooking.getSource().get(0).create();
                }else if(inputSlot == 1 && !InventoryUtils.isCustomItemsListEmpty(cooking.getCustomResults())){
                    return cooking.getCustomResult().create();
                }
                return itemStack;
            }
        }));
