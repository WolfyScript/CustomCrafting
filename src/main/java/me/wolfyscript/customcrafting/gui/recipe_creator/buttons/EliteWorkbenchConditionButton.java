                ...
                if (event.getClick().isRightClick()) {
                    ...
                    guiHandler.getCurrentInv().openChat("elite_workbench", guiHandler, (guiHandler1, player1, s, args) -> {
                        if (args.length > 1) {
                            ...
                            if (condition.getEliteWorkbenches().contains(customItem.getId())) {
                                window.sendMessage(player1, "already_existing");
                                return true;
                            }
                            ((EliteWorkbenchCondition) conditions.getByID("elite_workbench")).addEliteWorkbenches(customItem.getId());
                            recipeConfig.setConditions(conditions);
                            return false;
                        }
