...
class RecipeIndex {
    ...
    internal val byType: Multimap<RecipeType<*>, RecipeReference<*>> = Multimaps.newSetMultimap(Object2ObjectOpenHashMap()) { ObjectOpenHashSet() }

    fun values(): Collection<RecipeReference<*>> {
        return Collections.unmodifiableCollection(byKey.values)
    }

    fun <T: CustomRecipe<*,*>> register(key: Key, recipe: T) : RecipeReference<T> {
        synchronized(this) {
    ...
    fun get(key: Key): RecipeReference<*>? {
        synchronized(this) {
            return byKey[key]
        }
    }
