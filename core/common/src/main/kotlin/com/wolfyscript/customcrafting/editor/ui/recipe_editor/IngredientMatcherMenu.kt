...
private class IngredientMatcherStore() : Store() {
    ...
    private fun getAvailableMatchersList(fromIndex: Int, toIndex: Int) {
        storeCoroutineScope.launch {
            ...
            availableMatchers.update {
                val matchers = try {
                    val all = CustomCraftingRegistryTypes.ingredientMatcherModels.resolveOrThrow()
                        .values().toList()
        ...
        Row(Modifier.fillMaxWidth()) {
            ...
            if (selectingType) {
                ...
                Box(Modifier.height(4.slots).width(6.slots)) {
                    val ui = try {
                        CustomCraftingRegistryTypes.ingredientMatcherModels.resolveOrThrow()[matcher.typeKey]
                    } catch (e: Exception) {
                            ...
                            Button(onClick = {
                                ...
                                Icon(stack = ItemStack(Items.COMMAND_BLOCK).apply {
                                    set(
                                        ...
                                        CustomCraftingRegistryTypes.ingredientMatcherModels.resolveOrThrow()
