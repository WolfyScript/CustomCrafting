    ...
    fun initRegistries() {
        createRegistry(CustomCraftingRegistryTypes.recipeTypes) {
            RegistrySimple<RecipeType<*>>(it).apply {
                register(
                    RecipeTypes.crafting.key.key, RecipeTypeImpl(CustomRecipeCrafting::class.java, Items.CRAFTING_TABLE)
                )
                register(
                    RecipeTypes.cooking.key.key, RecipeTypeImpl(CustomRecipeCooking::class.java, Items.FURNACE)
                )
                register(
                    RecipeTypes.mixing.key.key, RecipeTypeImpl(CustomRecipeMixing::class.java, Items.CAULDRON)
                )
                register(
                    RecipeTypes.repairing.key.key, RecipeTypeImpl(CustomRecipeRepairing::class.java, Items.ANVIL)
                )
                register(
                    RecipeTypes.smithing.key.key, RecipeTypeImpl(CustomRecipeSmithing::class.java, Items.SMITHING_TABLE)
                )
                register(
                    RecipeTypes.stonecutting.key.key, RecipeTypeImpl(CustomRecipeStonecutting::class.java, Items.STONECUTTER)
                )
                register(
                    RecipeTypes.grinding.key.key, RecipeTypeImpl(CustomRecipeGrinding::class.java, Items.GRINDSTONE)
                )
            }
        }
